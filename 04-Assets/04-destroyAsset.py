...
from algosdk.transaction import AssetConfigTxn, write_to_file
...
def destroyAsset(managerMNEMFile,assetID,algodClient):
    ...
    write_to_file([txn],TXFolder+"04-assetDestroy.utxn")
    stxn=txn.sign(managerSK)
    txid=algodClient.send_transaction(stxn)
    print(f'{"TX id:":28s}{txid:s}')
    write_to_file([stxn],TXFolder+"04-assetDestroy.stxn")
