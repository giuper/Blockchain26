...
print(f'{"SK64 from mnem:":45s}{SK64:s}')
print(f'{"written in file:":45s}{fileSK64:s}\n')
with open(fileSK64,"w") as f:
    f.write(SK64)

SK=base64.b64decode(SK64)                               #obtain the secret key by based64 decoding SK64
print(f'{"SK from decoding SK64:":45s}{SK.hex():s}')
print(f'{"written in file:":45s}{fileSK:s}\n')
with open(fileSK,"wb") as f:
    f.write(SK)
print(f'{"private key from splitting SK:":45s}{SK[0:32].hex()}')  #first half is the actual secret key
print(f'{"public key from splitting SK:":45s}{SK[32:].hex()}\n')  #second half is the public key

pk25519=publickey(SK[0:32])                                       #obtain the public key through the ED25519 python module
print(f'{"public key from SK through ed25519new:":45s}{pk25519.hex():s}')
print(f'{"written in file:":45s}{filePKSDKed25519:s}\n')
...
print(f'{"Algorand address from SK64:":45s}{Addr:s}')
print(f'{"written in file:":45s}{fileAddr:s}')
with open(fileAddr,"w") as f:
    f.write(Addr)

AddrDec=base64.b32decode(Addr+'======')                #add padding as required by base32 encoding and then decode
print(f'{"Public key from Addr ":45s}{AddrDec[:-4].hex()}') #this will give the public key again
